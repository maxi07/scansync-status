...
class BitmapFont:
    ...
    def draw_text(self, fb, text, x, y, color, scale=1):
        for ch in text:
            glyph = FONT_5X8.get(ch, FONT_5X8.get(ch.upper(), FONT_5X8['?']))
            ...
            x += (len(glyph) + self.spacing) * scale
...
def render_display(epd, data, wifi_ok, server_ok, rssi=0):
    ...
    epd.text_big("ScanSync", 4, 3, 1, scale=2)
...
def main():
    ...
    while True:
        try:
            ...
            try:
                epd.fill(0xff)
                epd.text_big("FEHLER!", 100, 50, 0, scale=2)
